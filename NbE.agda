...
reflect {σ = 𝟙} {t} ne = ⟨⟩ , η-⟨⟩ ‣ ✦ , ⟨⟩ -- _ , ✦ , (‘ ne)
reflect {σ = σ ẋ τ} ne = let    c₁ = reflect (π₁ ne)
                         in let c₂ = reflect (π₂ ne)
                         in let (t₁ , π₁t→t₁ , n₁) = reify c₁
                         in let (t₂ , π₂t→t₂ , n₂) = reify c₂
                         in ((t₁ , t₂) , η-, ‣ ξ-,⋆ π₁t→t₁ π₂t→t₂ , n₁ , n₂) , c₁ , c₂
reflect {Γ} {σ ⇒ τ} {t} ne = let (t' , wk-t·ze→t' , nt') = reify (f (σ ∷ Γ) wk (` ze) (reflect (` ze))) 
                             in ((ƛ t') , η-ƛ ‣ ξ-ƛ⋆ wk-t·ze→t' , (ƛ nt'))
                             , f
    where
        f : (Θ : Cxt)(ρ : Ren Γ Θ)(a : Θ ⊢ σ) → Comp σ a → Comp τ (rename ρ t · a)
        f Θ ρ a c with reify c
        ... | a' , a→a' , na' = comp-backward⋆ _ (ξ-·⋆ ✦ a→a') (reflect (rename-ne ρ ne · na'))
