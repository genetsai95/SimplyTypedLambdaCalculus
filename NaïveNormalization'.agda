...
rename-comp {σ = 𝟙} ρ t (t' , t→t') = rename ρ t' , map-rename ρ t→t'
...
⟦ ƛ_ {τ = 𝟙} t ⟧ Δ ts cs = ((ƛ t) [ ts ]) , β-base , (ƛ subst t (ts ↑)) , β-base , 
                          λ Θ ρ s scs → let (t' , t[s∷mr-ts]→t' , t'' , t'→t'') = ⟦ t ⟧ Θ (s ∷ mapSub (rename ρ) ts) (scs ∷ renameˢ ρ cs) 
